...
## 3. 학습 목표
- Security Group/Key Pair 표준화 자동화 주제를 Ansible 중심으로 설명할 수 있다.
...
## 9. AI 페어링 가이드
- 사전 점검 프롬프트: Security Group/Key Pair 표준화 자동화 실습 전에 실패할 수 있는 지점 3개를 체크리스트로 만들어줘.
...
## 12. 실행 검증 결과 (자동 수집)
- 검증 일시: 자동 실행
- 실행 명령: `HOME=/home/Python_Ansible-Playbook ANSIBLE_LOCAL_TEMP=/home/Python_Ansible-Playbook/.ansible/tmp ANSIBLE_REMOTE_TEMP=/tmp/.ansible/tmp ANSIBLE_BECOME_ASK_PASS=False .venv/bin/ansible-playbook -i inventories/local/hosts.ini curriculum/lecture33/playbook.yml -e install_enabled=false`
...
- 실행 로그: `results/lecture-verification/logs/lecture33.log`

![lecture33 실행 캡처](../../results/lecture-verification/captures/lecture33.png)


## 13. 실행 검증 결과 (install_enabled=true 자동 수집)
- 검증 일시: 자동 실행
- 실행 명령: `HOME=/home/Python_Ansible-Playbook ANSIBLE_LOCAL_TEMP=/home/Python_Ansible-Playbook/.ansible/tmp ANSIBLE_REMOTE_TEMP=/tmp/.ansible/tmp ANSIBLE_BECOME_ASK_PASS=False .venv/bin/ansible-playbook -i inventories/local/hosts.ini curriculum/lecture33/playbook.yml -e install_enabled=true`
- 검증 상태: `PASS` (exit_code=0)
- RECAP: `localhost : ok=5 changed=1 unreachable=0 failed=0 skipped=0 rescued=0 ignored=0`
- 올바른 예상 결과:
  - `Gathering Facts` 성공
  - `Show lecture context` 출력 확인
...
- 실행 로그: `results/lecture-verification-install/logs/lecture33.log`

![lecture33 install_enabled=true 실행 캡처](../../results/lecture-verification-install/captures/lecture33.png)
