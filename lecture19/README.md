# lecture19 - Kolla Ansible 배포 준비와 검증
...
## 2. 상세 학습 내용
- Kolla Ansible 배포 전 사전 점검 항목을 정리한다.
- 컨테이너 기반 OpenStack 제어평면 구성 개념을 이해한다.
- 배포 후 검증 항목(keystone/nova/neutron/horizon)을 정의한다.
...
3. install_enabled=true로 docker 의존성 설치를 확인한다.
4. 서비스 검증 항목을 문서화한다.
5. 운영 반영 전 점검 절차를 확정한다.

## 5. 실행 방법 (프로젝트 루트에서 실행)
```bash
# 기본 검증 실행 (설치 제외)
ansible-playbook -i ansible/inventories/local/hosts.ini lecture19/playbook.yml -e install_enabled=false
